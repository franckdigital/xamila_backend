"""
GEK CAPITAL PDF Template with complete field mappings for all 95 annex fields.
"""
import os
...
class GEKCapitalTemplate(BasePDFTemplate):
    """
    PDF Template for GEK CAPITAL SA contract.
    Handles all 95 annex fields across pages 22, 23, and 26.
    """
    
    def get_template_path(self) -> str:
        """Return path to GEK CAPITAL PDF template."""
        return os.path.join(
            settings.BASE_DIR,
            'contracts',
    ...
    def fill_page(self, canvas_obj, page_index: int, context: Dict[str, Any]):
        """
        Fill specific pages with data for GEK CAPITAL template.
