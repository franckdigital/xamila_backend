...
class Cohorte(models.Model):
    ...
    code = models.CharField(
        ...
        unique=True, 
        blank=True,
        help_text="Code unique d'accès à la cohorte (généré automatiquement)"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        # Générer automatiquement le code au format COHORTE2024-01-Username
        if not self.code and self.user:
            mois_format = f"{self.mois:02d}"  # Format sur 2 chiffres (01, 02, etc.)
            username = self.user.username or self.user.email.split('@')[0]
            self.code = f"COHORTE{self.annee}-{mois_format}-{username}"
